...
namespace smartHookah.Controllers
{
    ...
    {
        public async Task<SmokeViewModel> CreateModel(
            SmartHookahContext db,
            int? dbSmokeSessionId = null,
            string redisSessionId = null,
            string reddisHookahId = null)
        {
            ...
            if (dbSmokeSessionId.HasValue) session = db.SmokeSessions.Find(dbSmokeSessionId);
            ...
            this.StandSetting = DeviceControlController.GetDeviceSettingViewModel(
                this.Hookah.Setting,
                this.Hookah.Version,
                db);
            this.State = await IotDeviceHelper.GetState(session.Hookah.Code);
            this.SessionId = session.SessionId;
            this.StandSetting.SessionId = this.SessionId;
            Person person = null;
            try
            {
            ...
            if (person != null) this.IsAssigned = session.IsPersonAssign(person.Id);
