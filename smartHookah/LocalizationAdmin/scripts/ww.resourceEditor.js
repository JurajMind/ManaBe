...
function removeResourceIcons () {
    $(window).unbind("resize.resize_ww_resourceeditor");
    $(".resource-editor-icon").remove();
}

function isResourceEditingEnabled() {
    if ($(".resource-editor-icon").length > 0)
        return true;
...
function showEditorForm(e) {
    ...
    window.open(adminUrl + "?ResourceSet=" + encodeURIComponent(resSet) +
        "&ResourceId=" + encodeURIComponent(resId) +
        "&Content=" + encodeURIComponent(content),
        editorWindowName, editorWindowOpenOptions);
}
