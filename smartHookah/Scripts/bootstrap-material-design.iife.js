	      ...
	      value: function _rejectWithoutRequiredClasses() {
	        ...
	        try {
	          for (var _iterator3 = this.config.requiredClasses[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
	            ...
	            // error if not found
	            if (!found) {
	              $.error(this.constructor.name + ' element: ' + Util$1.describe(this.$element) + ' requires class: ' + requiredClass);
	            }
	          }
	        } catch (err) {
	...
	var CollapseInline = function ($) {
	  ...
	  var CollapseInline = function (_Base) {
	    ...
	    function CollapseInline($element, config) {
	      ...
	      Util$1.assert(_this.$collapse, !_this.$collapse.hasClass(ClassName.COLLAPSE), Util$1.describe(_this.$collapse) + ' is expected to have the \'' + ClassName.COLLAPSE + '\' class.  It is being targeted by ' + Util$1.describe($element));
